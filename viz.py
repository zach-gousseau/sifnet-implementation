...
def create_time_timestep_matrix(preds, timestep_in_days=1):
    ...
    time_2d = (
        ...
        + np.array(
            [
                np.timedelta64(timestep_in_days * i, "D")
                for i in range(len(preds.timestep))
            ]
        )[:, None]
    )
    return time_2d
...
for i, timestep in enumerate(tqdm(range(60))):
    ...
    preds_timestep = xr.concat(
        [xr.open_dataset(fn).sel(timestep=timestep) for fn in fns], dim="time"
    )
    preds_timestep = preds_timestep.assign_coords({"launch_date": preds_timestep.time})
    preds_timestep = preds_timestep.assign_coords(
        {"time": preds_timestep.time.values + np.timedelta64(timestep, "D")}
    )
...
sns.heatmap(
    accuracies.T, yticklabels=[month_name[i][:3] for i in months]
)  # , vmin=0.8, vmax=1)
